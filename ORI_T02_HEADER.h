...
#define TAM_ARQUIVO_JOGADORES_IDX (1000 * MAX_REGISTROS + 1)
#define TAM_ARQUIVO_KITS_IDX (1000 * MAX_REGISTROS + 1)
#define TAM_ARQUIVO_PARTIDAS_IDX (1000 * MAX_REGISTROS + 1)
#define TAM_ARQUIVO_RESULTADOS_IDX (1000 * MAX_REGISTROS + 1)
#define TAM_DATA_PARTIDA_IDX (1000 * MAX_REGISTROS + 1)
#define TAM_PRECO_KIT_IDX (1000 * MAX_REGISTROS + 1)
#define TAM_ARQUIVO_JOGADOR_KIT_SECUNDARIO_IDX (1000 * MAX_REGISTROS + 1)
#define TAM_ARQUIVO_JOGADOR_KIT_PRIMARIO_IDX (1000 * MAX_REGISTROS + 1)
...
#define TAM_CHAVE_RESULTADOS_IDX ((TAM_ID_JOGADOR - 1) + (TAM_ID_PARTIDA - 1) + TAM_RRN_REGISTRO)
#define TAM_CHAVE_PRECO_KIT_IDX ((TAM_FLOAT_NUMBER - 1) + (TAM_ID_KIT - 1))
#define TAM_CHAVE_DATA_PARTIDA_IDX ((TAM_DATETIME - 1) + (TAM_ID_PARTIDA - 1))
...
typedef struct {
	char *arquivo_secundario; // Ponteiro para o arquivo de índice secundário
	char *arquivo_primario; // Ponteiro para o arquivo de índice primário
	...
	int (*compar)(const void *key, const void *elem); //Função utilizada para comparar as chaves do índice secundário, igual às funções de comparação do bsearch e qsort.
} inverted_list;

/* Struct para os parâmetros de uma Árvore-B */
typedef struct {
	int rrn_raiz; // RRN da raiz
	char *arquivo; // Ponteiro para o arquivo de índice
	unsigned qtd_nos; // Quantidade de nós no arquivo de índice
	unsigned tam_chave; // Tamanho de uma chave nesse índice
	int (*compar)(const void *key, const void *elem); //Função utilizada para comparar as chaves do índice, igual às funções de comparação do bsearch e qsort.
} btree;

typedef struct {
	char chave_promovida[100 + 1]; // TAM_CHAVE_NOME_PISTA_IDX é a maior chave possível
	int filho_direito;
} promovido_aux;
...
int order_info_jogador(const void *key, const void *elem);
...
/* Arquivos de índices */
char ARQUIVO_JOGADORES_IDX[TAM_ARQUIVO_JOGADORES_IDX];
char ARQUIVO_KITS_IDX[TAM_ARQUIVO_KITS_IDX];
char ARQUIVO_PARTIDAS_IDX[TAM_ARQUIVO_PARTIDAS_IDX];
char ARQUIVO_RESULTADOS_IDX[TAM_ARQUIVO_RESULTADOS_IDX];
char ARQUIVO_DATA_PARTIDA_IDX[TAM_DATA_PARTIDA_IDX];
char ARQUIVO_PRECO_KIT_IDX[TAM_PRECO_KIT_IDX];
char ARQUIVO_JOGADOR_KIT_SECUNDARIO_IDX[TAM_ARQUIVO_JOGADOR_KIT_SECUNDARIO_IDX];
char ARQUIVO_JOGADOR_KIT_PRIMARIO_IDX[TAM_ARQUIVO_JOGADOR_KIT_PRIMARIO_IDX];
...
btree jogadores_idx = {
	.rrn_raiz = -1,
	.arquivo = ARQUIVO_JOGADORES_IDX,
	.qtd_nos = 0,
	.tam_chave = TAM_CHAVE_JOGADORES_IDX,
	.compar = order_jogadores_idx,
};

btree kits_idx = {
	.rrn_raiz = -1,
	.arquivo = ARQUIVO_KITS_IDX,
	.qtd_nos = 0,
	.tam_chave = TAM_CHAVE_KITS_IDX,
	.compar = order_kits_idx,
};

btree partidas_idx = {
	.rrn_raiz = -1,
	.arquivo = ARQUIVO_PARTIDAS_IDX,
	.qtd_nos = 0,
	.tam_chave = TAM_CHAVE_PARTIDAS_IDX,
	.compar = order_partidas_idx,
};

btree resultados_idx = {
	.rrn_raiz = -1,
	.arquivo = ARQUIVO_RESULTADOS_IDX,
	.qtd_nos = 0,
	.tam_chave = TAM_CHAVE_RESULTADOS_IDX,
	.compar = order_resultados_idx,
};

btree data_partida_idx = {
	.rrn_raiz = -1,
	.arquivo = ARQUIVO_DATA_PARTIDA_IDX,
	.qtd_nos = 0,
	.tam_chave = TAM_CHAVE_DATA_PARTIDA_IDX,
	.compar = order_data_partida_idx,
};

btree preco_kit_idx = {
	.rrn_raiz = -1,
	.arquivo = ARQUIVO_PRECO_KIT_IDX,
	.qtd_nos = 0,
	.tam_chave = TAM_CHAVE_PRECO_KIT_IDX,
	.compar = order_preco_kit_idx,
};

inverted_list jogador_kits_idx = {
	.arquivo_secundario = ARQUIVO_JOGADOR_KIT_SECUNDARIO_IDX,
	.arquivo_primario = ARQUIVO_JOGADOR_KIT_PRIMARIO_IDX,
...
bool exibir_btree_kitss(char *chave);
bool exibir_btree_partidass(char *chave);
bool exibir_btree_resultados(char *chave);
